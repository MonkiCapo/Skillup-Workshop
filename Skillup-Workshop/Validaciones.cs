...
namespace Skillup_Workshop
{
    public class Validaciones
    {
        ...
        public static void Dificultad(string _dificultad, out string _dificultadBien)
        {
            string[] dificultades = { "Facil", "Intermedio", "Avanzado" };
            _dificultadBien = dificultades.FirstOrDefault(d => d.ToLower() == _dificultad.ToLower().Trim()) ?? throw new ArgumentException("Dificultad no encontrada");
        }
        public static void Estado(string estado, Alumno alumno)
        {
            ...
            if (estados.FirstOrDefault(e => e.ToLower() == estado) is null)
            {
        ...
        public static void Asignado(bool asignado)
        {
            if (asignado is false)
            { 
